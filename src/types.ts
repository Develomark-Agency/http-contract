...
export type Endpoint<Template extends string, PathKeys extends string, MethodSet extends boolean, Path, Query, Body, Headers, Output, Errors> = {
  ...
  path: [PathKeys] extends [never]
    ? never
    : <S extends StandardSchema<unknown, PathSchemaOutput<PathKeys>>>(schema: S) => Endpoint<Template, PathKeys, MethodSet, SchemaOutput<S>, Query, Body, Headers, Output, Errors>;
  ...
  }) => ValidateReturn>(fn: F): Endpoint<Template, PathKeys, MethodSet, Path, Query, Body, Headers, Output, Errors | ValidateError<ReturnType<F>>>;
  transform<F extends (ctx: RuntimeContext & {
    path: [Path] extends [never] ? Record<string, PathParamValue> : Path;
    query: [Query] extends [never] ? QueryInput : Query;
    headers: [Headers] extends [never] ? HeadersInput : Headers;
    value: Output;
  }) => ValidateReturn>(fn: F): Endpoint<Template, PathKeys, MethodSet, Path, Query, Body, Headers, TransformOk<ReturnType<F>, Output>, Errors | TransformError<ReturnType<F>>>;
};
